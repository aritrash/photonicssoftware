...
- Balanced ternary `Trit` type with values **−1, 0, +1**.[web:94]  
- Unary operations:
...
- Binary operations:
  ...
  - **TNAND** — ternary NAND defined as TNOT∘TAND, analogous to Boolean NAND[web:41][web:53]  
...
- Conversion between:
  ...
  - Jones vector ↔ **Stokes parameters** and **Poincaré‑sphere coordinates**[web:111][web:112][web:125]  
...
- `gratingdesign` module implements a simple **effective‑medium theory (EMT)** design workflow for 1D subwavelength gratings:
  ...
  - Outputs:
    ...
    - Slit width w  
    - Duty cycle f = w / Λ  
    - Effective indices \(n_{\text{eff,TE}}\), \(n_{\text{eff,TM}}\)[web:61][web:66][web:135]  
- Uses first‑order EMT formulas for TE/TM and sweeps duty cycle to maximize \(|n_{\text{eff,TE}} - n_{\text{eff,TM}}|\) as a crude polarization‑contrast figure of merit.[web:61][web:66][web:83]  
- Includes representative refractive index values for intrinsic Si and SiN in the near‑IR (easily replaceable with more accurate datasets).[web:74][web:85][web:81][web:90]  
...
- For an incident polarization angle θ:
  - Uses **Malus law** \(I = \eta\,I_0 \cos^2(\Delta\theta)\) to compute transmitted intensity per channel.[web:146][web:120][web:155]  
...
- `visualizationhelpers` module (Matplotlib) for:
  ...
  - **Poincaré sphere 3D plot**:
    ...
    - Canonical states and current output point via Stokes parameters[web:125][web:122]  
  - **2D Jones‑vector plot**:
    - Magnitudes and phases of Ex and Ey in side‑by‑side subplots  
- All exposed as functions returning `matplotlib.figure.Figure`, ready to be embedded in PyQt6 via `FigureCanvasQTAgg`.[web:59]  
...
5. Read off:
   ...
   - Slit width (nm)  
   - Duty cycle  
6. Use these as initial design guidelines for polarization‑selective gratings in your PICs.[web:61][web:66][web:135]  
...
- Physics models are **first‑order**:
  - Grating design uses simple EMT and a contrast metric; no full RCWA/FDTD calibration yet.[web:61][web:66][web:139][web:137]  
  - Triple‑channel detector is Malus‑law only, without noise or fabrication non‑idealities.[web:146][web:149]  
- Gate set is **minimal**:
